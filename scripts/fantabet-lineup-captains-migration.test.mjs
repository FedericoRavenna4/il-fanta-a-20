...
test("Admin usa select mobile full width limitate agli undici slot", () => { assert.match(adminUi, /Scegli tra gli 11 titolari/); assert.match(adminUi, /players=\{team\.players\}/); assert.match(adminUi, /min-h-12 w-full min-w-0/); assert.match(adminUi, /captainOrder/); });
