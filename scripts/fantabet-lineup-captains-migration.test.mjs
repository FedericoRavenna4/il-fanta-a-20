...
test("read pubblico espone C e VC nello snapshot", () => { assert.match(sql, /'captain',coalesce\(entry\.ordine=lineup\.captain_order,false\)/); assert.match(sql, /'viceCaptain',coalesce\(entry\.ordine=lineup\.vice_captain_order,false\)/); assert.match(publicUi, /player\.captain \? " \(C\)"/); assert.match(publicUi, /player\.viceCaptain \? " \(VC\)"/); });
