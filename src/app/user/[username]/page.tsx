...
import type { VerificationStatus } from "@/lib/account/verification";
...
export default async function PublicUserPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const { data: profile, error: profileError } = await supabase.from("profiles").select("id,username,societa_id,avatar_url").eq("username", username).maybeSingle();
