...
import UsernameForm from "@/app/account/UsernameForm";
...
type CompetitivePositionSummary = { fantabet_position: number | null; fantabet_points: number | null; arcade_position: number | null; arcade_level: number | null; arcade_meters: number | null };
...
export default async function PublicUserPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [{ data: auth }, supportResult, positionsResult, seasonResult, allTeams, userEmblemsResult] = await Promise.all([
    supabase.auth.getUser(),
    supabase.rpc("public_profile_support_summary", { p_profile_id: profile.id }),
    supabase.rpc("public_profile_competitive_positions", { p_profile_id: profile.id }),
  ...
  const competitivePositions = ((positionsResult.data ?? [])[0] ?? null) as CompetitivePositionSummary | null;
  ...
  {owner && <UsernameForm currentUsername={profile.username} />}
    ...
    <div data-profile-stats className="grid grid-cols-2 gap-2 sm:gap-4"><StatCard title="FantaBet" accent="bg-orange-400">{competitivePositions?.fantabet_position ? <><strong className="mt-3 block text-2xl font-black text-blue-950 sm:text-4xl">#{competitivePositions.fantabet_position}</strong><p className="mt-1 text-[10px] font-black uppercase text-slate-500 sm:text-sm">{competitivePositions.fantabet_points} PT</p></> : <p className="mt-4 text-xs font-bold text-slate-500 sm:text-sm">Non classificato</p>}</StatCard><StatCard title="Arcade" accent="bg-sky-500">{competitivePositions?.arcade_position && competitivePositions.arcade_level !== null && competitivePositions.arcade_meters !== null ? <><strong className="mt-3 block text-2xl font-black text-blue-950 sm:text-4xl">{competitivePositions.arcade_position}° posto</strong><p className="mt-1 text-[10px] font-black uppercase text-slate-500 sm:text-sm">L{competitivePositions.arcade_level} · {competitivePositions.arcade_meters} m</p></> : <p className="mt-4 text-xs font-bold text-slate-500 sm:text-sm">Non classificato</p>}</StatCard></div>
