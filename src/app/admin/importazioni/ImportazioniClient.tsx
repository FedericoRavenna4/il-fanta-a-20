...
import { createImportPreviewAction, deleteImportAction, deletePublishedCalendarAction, inspectCalendarDeletionAction, publishImportAction } from "./actions";
  ...
  return <div className="space-y-7">
    <form action={submitPreview} className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-black text-blue-950">Tipo importazione<select name="importType" className={field} value={importType} onChange={(event) => { setImportType(event.target.value as ImportType); resetCompetitionSelection(); }}><option value="calendario_campionato">Calendario campionato</option><option value="calendario_coppa">Coppe</option><option disabled>Rose — non ancora disponibili</option><option disabled>Mercato — non ancora disponibile</option></select></label>
