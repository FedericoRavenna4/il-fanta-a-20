...
const roseLeagueCodes = new Set(["serie-a", "serie-b", "serie-c-girone-a", "serie-c-girone-b", "serie-c-girone-c"]);

function roseLeagueCode(item: ImportHistoryItem) {
  if (!item.summary || typeof item.summary !== "object" || Array.isArray(item.summary)) return null;
  const value = String((item.summary as Record<string, unknown>).legaCodice ?? "").trim().toLowerCase();
  return roseLeagueCodes.has(value) ? value : null;
}
...
export default function ImportazioniClient({ catalog, history }: { catalog: AdminCatalog; history: ImportHistoryItem[] }) {
  ...
  const [roseDeleteTarget, setRoseDeleteTarget] = useState<{ item: ImportHistoryItem; season: string; league: string; players: number } | null>(null);
  ...
  const currentRoseImportIds = useMemo(() => {
    const seen = new Set<string>();
    const current = new Set<string>();
    for (const item of historyItems) {
      if (item.type !== "rose") continue;
      const league = roseLeagueCode(item);
      if (!league) continue;
      const key = `${item.season}:${league}`;
      if (seen.has(key)) continue;
      seen.add(key);
      if (["pubblicata", "pubblicata_con_warning"].includes(item.status)) current.add(item.id);
    }
    return current;
  }, [historyItems]);
  useEffect(() => {
  ...
  function requestRoseDeletion(item: ImportHistoryItem) {
    ...
    startTransition(async () => {
      ...
      if (!result.ok) { setMessage(result.message); return; }
      setRoseDeleteTarget({ item, season: result.season, league: result.league, players: result.players });
    });
  }
  ...
  return <div className="space-y-7">
    ...
    {preview && <section className="space-y-4" aria-labelledby="preview-title">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">{Object.entries(preview.importType === "rose" ? { Società: preview.summary.societaRiconosciute, Calciatori: preview.summary.calciatori, Valide: preview.summary.calciatori, Aggiunti: preview.summary.insert, Aggiornati: preview.summary.update, Trasferiti: preview.summary.trasferimenti, Rimossi: preview.summary.rimossi, Warning: preview.summary.warning, Errori: preview.summary.error } : { Giornate: preview.summary.giornate, Partite: preview.summary.partite, Riposi: preview.summary.riposi, Riconosciute: preview.summary.societaRiconosciute, "Non riconosciute": preview.summary.societaNonRiconosciute.length, Inserimenti: preview.summary.insert, Aggiornamenti: preview.summary.update, Invariate: preview.summary.unchanged, Rimossi: preview.summary.rimossi, Warning: preview.summary.warning, Errori: preview.summary.error }).map(([label,value]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-3"><p className="text-[10px] font-black uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 text-xl font-black text-blue-950">{value}</p></div>)}</div>
      {preview.importType === "rose" && <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold text-amber-950">Questa importazione sincronizzerà esclusivamente la lega {preview.targetLeagueCode ?? "non determinata"} della stagione selezionata.</div>}
