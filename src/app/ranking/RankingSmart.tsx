  ...
  return (
    <>
      <div className="grid gap-3 sm:gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        {podio[0]?.team && (
          <Link href={`/societa/${podio[0].team.slug}`} className="group relative overflow-hidden rounded-[2rem] border border-amber-300/40 bg-[linear-gradient(145deg,#17376f,#081f46)] p-3 text-white shadow-xl shadow-blue-950/15 transition hover:-translate-y-1 sm:p-7">
            ...
            <div className="relative grid min-h-0 grid-cols-[minmax(0,1fr)_64px] items-center gap-2 sm:min-h-52 sm:grid-cols-[1fr_150px] sm:gap-5">
              ...
              <Image src={podio[0].team.logo} alt={podio[0].team.nome} width={145} height={145} className="mx-auto max-h-16 w-auto object-contain drop-shadow-[0_16px_22px_rgba(0,0,0,0.3)] transition group-hover:scale-105 sm:max-h-36" />
            </div>
          </Link>
        )}
        <div className="grid gap-2 sm:grid-cols-2 sm:gap-4 lg:grid-cols-1">
          {podio.slice(1).map((row) => (
            <Link key={row.posizione} href={row.team ? `/societa/${row.team.slug}` : "#"} className="group grid min-h-0 grid-cols-[36px_minmax(0,1fr)_52px] items-center gap-2 rounded-[1.25rem] border border-slate-200 bg-white/85 p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg sm:min-h-24 sm:grid-cols-[52px_1fr_76px] sm:gap-3 sm:rounded-[1.5rem] sm:p-4">
              <p className="text-xl font-black text-slate-300 sm:text-3xl">0{row.posizione}</p><div><h3 className="line-clamp-2 text-xs font-black uppercase text-blue-950 sm:text-sm">{row.team?.nome ?? row.nomeRanking}</h3><p className="mt-0.5 text-[10px] font-bold text-slate-400 sm:mt-1 sm:text-xs">{row.puntiRanking} punti</p></div>{row.team && <Image src={row.team.logo} alt={row.team.nome} width={70} height={70} className="max-h-12 max-w-12 object-contain transition group-hover:scale-105 sm:max-h-16 sm:max-w-16" />}
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 shadow-xl shadow-slate-200/60">
        ...
        {rows.slice(0, 10).map((row, index) => (
          <Link key={row.posizione} href={row.team ? `/societa/${row.team.slug}` : "#"} className="grid min-w-0 grid-cols-[32px_minmax(0,1fr)_48px] items-center gap-2 border-t border-slate-100 px-3 py-2 transition hover:bg-sky-50/60 sm:grid-cols-[52px_minmax(0,1fr)_auto] sm:gap-3 sm:px-5 sm:py-3 lg:grid-cols-[60px_minmax(0,1fr)_90px_150px_70px_80px] lg:px-6">
            <span className="text-sm font-black text-blue-950">{row.posizione}°</span><div className="flex min-w-0 items-center gap-2 sm:gap-3">{row.team && <Image src={row.team.logo} alt="" width={42} height={42} className="h-9 w-9 shrink-0 object-contain sm:h-10 sm:w-10" />}<span className="min-w-0 text-xs font-black uppercase text-blue-950 sm:text-base"><AutoScrollName name={row.team?.nome ?? row.nomeRanking} index={index} /></span></div><span className="hidden text-sm font-bold text-slate-500 lg:block">{row.team?.stagioneIngresso ?? "—"}</span><span className="hidden truncate text-sm font-bold text-slate-500 lg:block">{row.team?.legaAttuale ?? "—"}</span><span className="hidden text-right text-sm font-black text-blue-950 lg:block">{row.trofei.totaleTrofei}</span><span className="text-right text-base font-black text-blue-950">{row.puntiRanking}</span>
          </Link>
        ))}
              ...
              {classifica.map((row, index) => (
                <div key={row.posizione} data-ranking-row data-fascia={fascia(row.posizione)}>
                  ...
                  <Link href={row.team ? `/societa/${row.team.slug}` : "#"} className="grid grid-cols-[44px_minmax(0,1fr)_52px_60px] items-center gap-2 border-b border-slate-100 px-2 py-3 transition hover:bg-white lg:grid-cols-[60px_minmax(220px,1fr)_90px_150px_65px_75px] lg:gap-3 lg:px-4">
                    ...
                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">{row.team && <Image src={row.team.logo} alt="" width={40} height={40} className="h-8 w-8 shrink-0 object-contain sm:h-10 sm:w-10" />}<span className="min-w-0 text-[11px] font-black uppercase text-blue-950 sm:text-base"><AutoScrollName name={row.team?.nome ?? row.nomeRanking} index={index + 10} /></span></div>
