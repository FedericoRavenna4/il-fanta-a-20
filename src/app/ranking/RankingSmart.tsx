      ...
      <div className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 shadow-xl shadow-slate-200/60">
        ...
        {rows.slice(0, 10).map((row, index) => (
          <Link key={row.posizione} href={row.team ? `/societa/${row.team.slug}` : "#"} className="grid min-w-0 grid-cols-[32px_minmax(0,1fr)_48px] items-center gap-2 border-t border-slate-100 px-3 py-2 transition hover:bg-sky-50/60 sm:grid-cols-[52px_minmax(0,1fr)_auto] sm:gap-3 sm:px-5 sm:py-3 lg:grid-cols-[60px_minmax(0,1fr)_90px_150px_70px_80px] lg:px-6">
            <span className="text-sm font-black text-blue-950">{row.posizione}°</span><div className="flex min-w-0 items-center gap-2 sm:gap-3">{row.team && <Image src={row.team.logo} alt="" width={42} height={42} className="h-9 w-9 shrink-0 object-contain sm:h-10 sm:w-10" />}<span className="min-w-0 text-[13px] font-black uppercase text-blue-950 sm:text-base"><AutoScrollName name={row.team?.nome ?? row.nomeRanking} index={index} /></span></div><span className="hidden text-sm font-bold text-slate-500 lg:block">{row.team?.stagioneIngresso ?? "—"}</span><span className="hidden truncate text-sm font-bold text-slate-500 lg:block">{row.team?.legaAttuale ?? "—"}</span><span className="hidden text-right text-sm font-black text-blue-950 lg:block">{row.trofei.totaleTrofei}</span><span className="text-right text-base font-black text-blue-950">{row.puntiRanking}</span>
          </Link>
        ))}
              ...
              {classifica.map((row, index) => (
                <div key={row.posizione} data-ranking-row data-fascia={fascia(row.posizione)}>
                  ...
                  <Link href={row.team ? `/societa/${row.team.slug}` : "#"} className="grid grid-cols-[44px_minmax(0,1fr)_52px_60px] items-center gap-2 border-b border-slate-100 px-2 py-3 transition hover:bg-white lg:grid-cols-[60px_minmax(220px,1fr)_90px_150px_65px_75px] lg:gap-3 lg:px-4">
                    ...
                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">{row.team && <Image src={row.team.logo} alt="" width={40} height={40} className="h-8 w-8 shrink-0 object-contain sm:h-10 sm:w-10" />}<span className="min-w-0 text-xs font-black uppercase text-blue-950 sm:text-base"><AutoScrollName name={row.team?.nome ?? row.nomeRanking} index={index + 10} /></span></div>
