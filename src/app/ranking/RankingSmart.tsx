  ...
  return (
    <>
      <div className="grid gap-3 sm:gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        {podio[0]?.team && (
          <article className="relative overflow-hidden rounded-[2rem] border border-amber-300/40 bg-[linear-gradient(145deg,#17376f,#081f46)] p-3 text-white shadow-xl shadow-blue-950/15 sm:p-7">
            <div className="absolute right-0 top-0 h-52 w-52 bg-amber-300/12 blur-[65px]" />
            <div className="relative grid min-h-0 grid-cols-[minmax(0,1fr)_64px] items-center gap-2 sm:min-h-52 sm:grid-cols-[1fr_150px] sm:gap-5">
              <div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-amber-300 sm:text-[10px] sm:tracking-[0.24em]">Ranking leader</p><Link href={`/societa/${podio[0].team.slug}`} className="group/name mt-2 inline-block rounded outline-none focus-visible:ring-2 focus-visible:ring-amber-300 sm:mt-3"><h3 className="text-xl font-black uppercase leading-tight transition group-hover/name:text-amber-300 sm:text-3xl">{podio[0].team.nome}</h3></Link><div className="mt-3 flex flex-wrap gap-1.5 sm:mt-6 sm:gap-3"><span className="rounded-full border border-white/10 bg-white/[0.06] px-2 py-1 text-[10px] font-black sm:px-3 sm:py-1.5 sm:text-xs">{podio[0].puntiRanking} punti</span><span className="rounded-full border border-white/10 bg-white/[0.06] px-2 py-1 text-[10px] font-black sm:px-3 sm:py-1.5 sm:text-xs">{podio[0].trofei.totaleTrofei} trofei</span></div></div>
              <Link href={`/societa/${podio[0].team.slug}`} aria-label={`Apri la scheda di ${podio[0].team.nome}`} className="group/logo mx-auto rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-amber-300"><Image src={podio[0].team.logo} alt={podio[0].team.nome} width={145} height={145} className="max-h-16 w-auto object-contain drop-shadow-[0_16px_22px_rgba(0,0,0,0.3)] transition group-hover/logo:scale-105 sm:max-h-36" /></Link>
            </div>
          </article>
        )}
        <div className="grid gap-2 sm:grid-cols-2 sm:gap-4 lg:grid-cols-1">
          {podio.slice(1).map((row) => (
            <article key={row.posizione} className="grid min-h-0 grid-cols-[36px_minmax(0,1fr)_52px] items-center gap-2 rounded-[1.25rem] border border-slate-200 bg-white/85 p-3 shadow-sm sm:min-h-24 sm:grid-cols-[52px_1fr_76px] sm:gap-3 sm:rounded-[1.5rem] sm:p-4">
              <p className="text-xl font-black text-slate-300 sm:text-3xl">0{row.posizione}</p><div>{row.team ? <Link href={`/societa/${row.team.slug}`} className="group/name inline-block rounded outline-none focus-visible:ring-2 focus-visible:ring-blue-900"><h3 className="line-clamp-2 text-xs font-black uppercase text-blue-950 transition group-hover/name:text-sky-700 sm:text-sm">{row.team.nome}</h3></Link> : <h3 className="line-clamp-2 text-xs font-black uppercase text-blue-950 sm:text-sm">{row.nomeRanking}</h3>}<p className="mt-0.5 text-[10px] font-bold text-slate-400 sm:mt-1 sm:text-xs">{row.puntiRanking} punti</p></div>{row.team && <Link href={`/societa/${row.team.slug}`} aria-label={`Apri la scheda di ${row.team.nome}`} className="group/logo rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-blue-900"><Image src={row.team.logo} alt={row.team.nome} width={70} height={70} className="max-h-12 max-w-12 object-contain transition group-hover/logo:scale-105 sm:max-h-16 sm:max-w-16" /></Link>}
            </article>
          ))}
        </div>
      </div>
      ...
      <div className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 shadow-xl shadow-slate-200/60">
        ...
        {rows.slice(0, 10).map((row, index) => (
          <div key={row.posizione} className="grid min-w-0 grid-cols-[32px_minmax(0,1fr)_48px] items-center gap-2 border-t border-slate-100 px-3 py-2 sm:grid-cols-[52px_minmax(0,1fr)_auto] sm:gap-3 sm:px-5 sm:py-3 lg:grid-cols-[60px_minmax(0,1fr)_90px_150px_70px_80px] lg:px-6">
            <span className="text-sm font-black text-blue-950">{row.posizione}°</span>
            {row.team ? (
              <Link href={`/societa/${row.team.slug}`} className="group/team flex min-w-0 items-center gap-2 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-blue-900 sm:gap-3">
        ...
        <div role="dialog" aria-modal="true" aria-label="Classifica completa del ranking" className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/65 p-2 backdrop-blur-md sm:p-4" onMouseDown={() => setOpen(false)}>
          <div className="max-h-[calc(100dvh-1rem)] w-full max-w-6xl overflow-y-auto overscroll-contain rounded-[1.5rem] bg-[#f8fbff] shadow-2xl sm:max-h-[90dvh] sm:rounded-[2rem]" onMouseDown={(event) => event.stopPropagation()} onScroll={updateFascia}>
            <div className="border-b border-slate-200 bg-white/95 px-4 pb-4 pt-4 sm:px-6 sm:pb-5 sm:pt-5">
              <div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.15em] text-amber-500 sm:text-[10px] sm:tracking-[0.2em]">Il Ranking</p><h2 className="mt-1 text-xl font-black uppercase text-blue-950 sm:text-2xl">Classifica completa</h2></div><button onClick={() => setOpen(false)} className="min-h-11 shrink-0 rounded-full border border-slate-200 px-4 py-2 text-xs font-black uppercase text-blue-950">Chiudi</button></div>
            ...
            {!search && <div ref={modalHeaderRef} className="sticky top-0 z-20 border-b border-slate-200/90 bg-[#f8fbff]/95 px-5 py-2.5 shadow-[0_12px_24px_-24px_rgba(7,31,69,0.85)] backdrop-blur-xl sm:px-7 sm:py-3 lg:px-11">
                <div className="mb-2 flex items-center gap-2.5">
                  ...
                  <span className="ml-auto text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-[10px]">
                    Posizioni {(fasciaAttiva - 1) * 20 + 1}-{fasciaAttiva * 20}
                  </span>
                </div>
                <div className="grid grid-cols-[44px_minmax(0,1fr)_52px_60px] items-center gap-2 text-[8px] font-black uppercase tracking-[0.1em] text-slate-400 sm:text-[9px] lg:grid-cols-[60px_minmax(220px,1fr)_90px_150px_65px_75px] lg:gap-3 lg:tracking-[0.14em]">
                  <span>Pos.</span>
                  <span>Società</span>
            ...
            <div className="p-3 sm:p-5 lg:p-7">
              {classifica.map((row, index) => (
                <div key={row.posizione} data-ranking-row data-fascia={fascia(row.posizione)}>
                  ...
                  <div className="grid grid-cols-[44px_minmax(0,1fr)_52px_60px] items-center gap-2 border-b border-slate-100 px-2 py-3 lg:grid-cols-[60px_minmax(220px,1fr)_90px_150px_65px_75px] lg:gap-3 lg:px-4">
