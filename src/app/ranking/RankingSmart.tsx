    ...
    <>
      ...
      <div className="mt-6 overflow-hidden rounded-[2rem] border border-slate-200 bg-white/90 shadow-xl shadow-slate-200/60">
        ...
        {rows.slice(0, 10).map((row) => (
          <Link key={row.posizione} href={row.team ? `/societa/${row.team.slug}` : "#"} className="grid grid-cols-[52px_1fr_auto] items-center gap-3 border-t border-slate-100 px-5 py-3 transition hover:bg-sky-50/60 lg:grid-cols-[60px_minmax(0,1fr)_90px_150px_70px_80px] lg:px-6">
        ...
        <div className="border-t border-slate-200 p-5 text-center"><button type="button" onClick={() => { setFasciaAttiva(1); setOpen(true); }} className="rounded-full bg-blue-950 px-7 py-3 text-xs font-black uppercase tracking-[0.14em] text-white transition hover:bg-blue-800">Vedi tutta la classifica del Ranking</button></div>
      </div>
      ...
      {open && (
        <div role="dialog" aria-modal="true" aria-label="Classifica completa del ranking" className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/65 p-2 backdrop-blur-md sm:p-4" onMouseDown={() => setOpen(false)}>
          <div className="max-h-[96vh] w-full max-w-6xl overflow-y-auto rounded-[1.5rem] bg-[#f8fbff] shadow-2xl sm:max-h-[90vh] sm:rounded-[2rem]" onMouseDown={(event) => event.stopPropagation()} onScroll={updateFascia}>
            <div ref={modalHeaderRef} className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 pb-3 pt-4 backdrop-blur-xl sm:px-6 sm:pb-4 sm:pt-5">
              <div className="flex items-center justify-between gap-5"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500">Classifica completa</p><h2 className="mt-1 text-2xl font-black uppercase text-blue-950">Il Ranking</h2></div><button onClick={() => setOpen(false)} className="rounded-full border border-slate-200 px-4 py-2 text-xs font-black uppercase text-blue-950">Chiudi</button></div>
              ...
              {classifica.map((row, index) => (
                <div key={row.posizione} data-ranking-row data-fascia={fascia(row.posizione)}>
                  ...
                  <Link href={row.team ? `/societa/${row.team.slug}` : "#"} className="grid grid-cols-[44px_minmax(0,1fr)_52px_60px] items-center gap-2 border-b border-slate-100 px-2 py-3 transition hover:bg-white lg:grid-cols-[60px_minmax(220px,1fr)_90px_150px_65px_75px] lg:gap-3 lg:px-4">
                    ...
                    <div className="flex min-w-0 items-center gap-3">{row.team && <Image src={row.team.logo} alt="" width={40} height={40} className="h-10 w-10 object-contain" />}<span className="truncate text-base font-black uppercase text-blue-950">{row.team?.nome ?? row.nomeRanking}</span></div>
