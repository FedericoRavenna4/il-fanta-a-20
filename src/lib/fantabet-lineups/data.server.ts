...
export async function getLineupMatchContext(matchId: number) {
  ...
  const admin = db();
  const [teamsResult, rostersResult, allTeamsResult] = await Promise.all([admin.from("societa").select("id,nome_ufficiale,nome_personalizzato,nome_normalizzato,slug").in("id", [match.home.id, match.away.id]), admin.from("rose_giocatori").select("id,lega_codice,societa_id,giocatore,squadra_reale,ruolo").eq("stagione_id", round.seasonId).order("giocatore"), admin.from("societa").select("id,nome_ufficiale,nome_personalizzato")]);
  if (teamsResult.error || rostersResult.error || allTeamsResult.error) throw new Error("LINEUP_CONTEXT_LOAD_FAILED");
  const rosters = (rostersResult.data ?? []) as Array<{ id: number; lega_codice: string; societa_id: number; giocatore: string; squadra_reale: string | null; ruolo: string }>;
  ...
  const option = (societyId: number): TeamOption => { const team = byId.get(societyId); if (!team) throw new Error("LINEUP_CONTEXT_LOAD_FAILED"); const leagues = [...new Set(rosters.filter((player) => player.societa_id === societyId).map((player) => player.lega_codice))]; if (leagues.length !== 1) throw new Error("LINEUP_LEAGUE_INVALID"); const leagueCode=leagues[0]; const mapPlayer=(player: typeof rosters[number]) => ({ id: player.id, name: player.giocatore, role: player.ruolo, societyId: player.societa_id, societyName: teamNames.get(player.societa_id) ?? "Società", realTeam: player.squadra_reale }); return { id: societyId, name: publicName(team), aliases: [team.nome_ufficiale, team.nome_personalizzato ?? "", team.nome_normalizzato, team.slug].filter(Boolean), leagueCode, roster: rosters.filter((player) => player.societa_id === societyId).map(mapPlayer), leaguePlayers: rosters.filter((player) => player.lega_codice === leagueCode).map(mapPlayer) }; };
