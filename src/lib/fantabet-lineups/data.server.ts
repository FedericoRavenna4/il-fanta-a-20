...
export async function getLineupMatchContext(matchId: number) {
  ...
  const admin = db();
  const [teamsResult, rostersResult] = await Promise.all([admin.from("societa").select("id,nome_ufficiale,nome_personalizzato,nome_normalizzato,slug").in("id", [match.home.id, match.away.id]), admin.from("rose_giocatori").select("id,societa_id,giocatore,ruolo").eq("stagione_id", round.seasonId).in("societa_id", [match.home.id, match.away.id]).order("giocatore")]);
  if (teamsResult.error || rostersResult.error) throw new Error("LINEUP_CONTEXT_LOAD_FAILED");
  ...
  const option = (societyId: number): TeamOption => { const team = byId.get(societyId); if (!team) throw new Error("LINEUP_CONTEXT_LOAD_FAILED"); return { id: societyId, name: publicName(team), aliases: [team.nome_ufficiale, team.nome_personalizzato ?? "", team.nome_normalizzato, team.slug].filter(Boolean), roster: rosters.filter((player) => player.societa_id === societyId).map((player) => ({ id: player.id, name: player.giocatore, role: player.ruolo })) }; };
