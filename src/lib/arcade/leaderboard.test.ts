...
const entry = (overrides: Partial<ArcadeLeaderboardEntry> = {}): ArcadeLeaderboardEntry => ({
  id: "entry-1",
  identityKey: "account:11111111-1111-4111-8111-111111111111",
...
test("stesso account conserva soltanto il record migliore", () => {
...
test("account diversi non vengono uniti anche con lo stesso nickname", () => {
  const first = entry({ id: "first" });
  const second = entry({ id: "second", identityKey: "account:22222222-2222-4222-8222-222222222222" });
  assert.equal(deduplicateArcadeLeaderboard([first, second]).length, 2);
});

test("cambiare username o società non cambia l'identità del record account", () => {
