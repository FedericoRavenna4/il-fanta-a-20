...
export async function getFantaBetAdminData(): Promise<FantaBetAdminData> {
  ...
  const [seasonResult, editionResult, roundResult, companyResult] = await Promise.all([
    ...
    db.from("fantabet_rounds").select("id,stagione_id,numero_giornata,opens_at,deadline_at,updated_at,status,required_predictions").order("deadline_at", { ascending: false }),
  ...
  const adminRounds = rounds.map((round): AdminRound => {
    ...
    return { id, stagioneId: Number(round.stagione_id), stagione: seasonMap.get(Number(round.stagione_id)) ?? String(round.stagione_id), giornata: Number(round.numero_giornata), opensAt: String(round.opens_at), deadlineAt: String(round.deadline_at), updatedAt: String(round.updated_at), status: String(round.status), required: Number(round.required_predictions), bets: roundBets.map((bet) => ({ id: Number(bet.id), partitaId: Number(bet.partita_id), type: String(bet.bet_type), points: Number(bet.points_value), order: Number(bet.display_order) })), participants, complete, confirmed, completion: roundCompletion(participants, confirmed), resultsAvailable: roundBets.length === 5 && roundBets.every((bet) => matchMap.get(Number(bet.partita_id))?.stato === "calcolata") };
  });
