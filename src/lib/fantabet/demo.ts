...
function roundBets(roundNumber: number, historical: boolean): FantaBetBetView[] {
  ...
  return types.map((type, index) => ({ id: roundNumber * 100 + index, type, points: type === "RISULTATO_ESATTO" ? 10 : type === "FANTAPUNTEGGIO_1X2" ? 2 : type === "1X2" ? 3 : 1, order: index + 1, home: teams[index * 2], away: teams[index * 2 + 1], homeStats: stats(index * 2), awayStats: stats(index * 2 + 1), homeRoster: [], awayRoster: [], result: historical ? index === 4 && roundNumber === 37 ? { status: "rinviata", homeGoals: null, awayGoals: null, homeFantasyPoints: null, awayFantasyPoints: null } : { status: "calcolata", homeGoals: [2, 1, 3, 1, 0][index], awayGoals: [1, 1, 0, 1, 2][index], homeFantasyPoints: [72, 68, 74, 69, 66][index], awayFantasyPoints: [67, 68, 65, 69, 71][index] } : { status: "programmata", homeGoals: null, awayGoals: null, homeFantasyPoints: null, awayFantasyPoints: null } }));
}
