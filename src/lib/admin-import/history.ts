...
type SeasonRow = { id: unknown; codice?: unknown };
...
export function buildImportHistory(rows: ImportRow[], editions: EditionRow[], competitions: CompetitionRow[], seasons: SeasonRow[] = []): ImportHistoryItem[] {
  ...
  const seasonMap = new Map(seasons.map((item) => [String(item.id), item]));
  return rows.map((item) => {
    ...
    return {
      ...
      total: Number(item.righe_totali),
