...
export function validateImportFile<T extends ImportFileLike>(file: T | null): T {
  if (!file || file.size === 0) throw new Error("Seleziona un file Excel non vuoto.");
  ...
  if (!EXTENSIONS.has(extension)) throw new Error("Formato non valido: sono ammessi soltanto file .xlsx e .xls.");
  if (file.size > MAX_IMPORT_FILE_BYTES) throw new Error("Il file supera il limite massimo di 10 MB.");
  if (!MIME_TYPES.has(file.type.toLocaleLowerCase("it"))) throw new Error("MIME type del file Excel non riconosciuto.");
  return file;
}
